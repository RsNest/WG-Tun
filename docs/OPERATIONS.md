...
```bash
transitforge-controller --config /etc/transitforge/controller.yaml
# lab:
transitforge-controller --plain-http --listen 127.0.0.1:8080 --data-dir ./data
```
...
```bash
transitforge-controller --config /etc/transitforge/controller.yaml --ui-listen 127.0.0.1:8444
```

Then browse `http://127.0.0.1:8444`. Sign in with an operator or readonly API token (file `auth.bootstrap_token_file` or a minted token). The UI stores the token only in memory, tied to a signed HTTP-only cookie (`transitforge_ui`, 12h). Restarting the controller invalidates all UI sessions. Logout clears the cookie.
...
```bash
transitforge-agent --config /etc/transitforge/agent.yaml
```
...
State directory default: `/run/transitforge` (`transport-state.json`, backups).

## CLI

```bash
transitforge --controller https://127.0.0.1:8443 --token-file ./data/bootstrap.token --insecure \
  apply --node ru-edge-1 --dry-run
```

Env (flags win): `TRANSITFORGE_CONTROLLER`, `TRANSITFORGE_TOKEN`, `TRANSITFORGE_TOKEN_FILE`, `TRANSITFORGE_INSECURE`.
...
`transitforge-haproxy-reload.path` watches `PathModified` and `PathChanged` on `/etc/haproxy/haproxy.cfg` (`PathChanged` is required because the agent replaces the file by rename). It starts `transitforge-haproxy-reload.service` (oneshot). The script:

1. Takes `/run/transitforge/haproxy-reload.lock` (`flock`) so `haproxy -c` and `systemctl reload` never overlap. A trigger that arrives during a run waits, then does a follow-up pass.
...
3. No-ops if the SHA-256 matches `/var/lib/transitforge/haproxy-reload.state` (touch / identical rewrite).
4. Validates with `haproxy -c -f /etc/haproxy/haproxy.cfg`. On failure it restores `/var/lib/transitforge/haproxy.last-good.cfg` over the live path and does **not** reload.
5. Snapshots last-good into `/var/lib/transitforge/haproxy-backups/`, then `systemctl reload haproxy`. Reload failure also restores last-good. Success updates last-good and the state hash.

Journal lines are prefixed `transitforge-haproxy-reload:` with `event=` (`change_detected`, `debounce_window_started`, `debounce_window_reset`, `validation_started`, `validation_passed`, `validation_failed`, `reload_started`, `reload_succeeded`, `reload_failed`, `restore_on_failure_triggered`, `reload_skipped`).

```bash
systemctl status transitforge-haproxy-reload.path
systemctl status transitforge-haproxy-reload.service
journalctl -u transitforge-haproxy-reload.service -e
journalctl -t transitforge-haproxy-reload -e
journalctl -u transitforge-haproxy-reload.service | grep event=
```

Force a pass (same script, useful after a manual edit):

```bash
sudo systemctl start transitforge-haproxy-reload.service
```

`haproxy-reload.state` and `haproxy.last-good.cfg` live under `/var/lib/transitforge` and survive reboot (idempotency). `/run/transitforge/haproxy-reload.debounce` is tmpfs and resets on reboot, which is intended. Snapshot into `haproxy-backups/` does not move last-good; last-good and the state hash are committed only after `reload_succeeded`. If systemd hits `TimeoutStartSec=120` (SIGKILL) between snapshot and reload, the next trigger retries the new file against the still-old last-good.
...
That installs Docker Engine, HAProxy, WireGuard tools, the path unit (not the live agent), then runs `scripts/smoke-test-debounce.sh` and `scripts/smoke-test-restore.sh`. Ctrl-C restores a valid `/etc/haproxy/haproxy.cfg`. A stamp is written to `/var/lib/transitforge/smoke-tests.passed` only if both tests pass.
...
# env TRANSITFORGE_VERSION=sha-abcdef1 bash scripts/enable-live-overlay.sh
# or, if you already minted an agent token (do not paste the value into chat/docs):
# TRANSITFORGE_AGENT_TOKEN='...' bash scripts/enable-live-overlay.sh
```
